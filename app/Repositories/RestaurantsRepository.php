...
class RestaurantsRepository
{
    ...
    public function getRecommendations(array $filters): array
    {
        ...
        $meal = $mealRepo->findBy('name', $filters['meal_name']);

        if(! $meal)
            return [];

        $this->mealRestaurantIds = $this->getByMealId($meal->id);
    ...
    private function getByMealId($mealId)
    {
        /*return $this->model
            ->meals()
            ->wherePivot('meal_id', $mealId)
            ->wherePivot('restaurant_id', 'NOT NULL')
            ->orderBy('restaurant_meal.meal_recommendations_count', 'desc')
            ->get();*/
        return RestaurantMeal::where('meal_id', $mealId)
